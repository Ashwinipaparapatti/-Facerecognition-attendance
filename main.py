...
if connection:
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if name == "Unknown":
                # If the face is unknown, prompt for a name and save the snapshots
                new_name = input("Enter name for the new face: ")
                known_face_encodings.append(face_encoding)
                known_face_names.append(new_name)

                # Save 10 snapshots of the new face
                for i in range(10):
                    snapshot_filename = os.path.join(image_folder_path, f"{new_name}_{i+1}.jpg")
                    cv2.imwrite(snapshot_filename, frame[y:y+h, x:x+w])
                    print(f"Snapshot {i+1} for {new_name} saved.")

                # Save the face encoding to the folder (optional)
                snapshot_filename = os.path.join(image_folder_path, f"{new_name}.jpg")
                cv2.imwrite(snapshot_filename, frame[y:y+h, x:x+w])

            else:
                # If the face is recognized and not already stored today
